...
worktree and a single status token; the rows below are the worktrees this session
spawned — across whatever repos they live in — each with its repo, its name, the
same token, and its GitHub PR state. The cross-repo lookups run detached and
cached, so the bar never stalls the prompt.
