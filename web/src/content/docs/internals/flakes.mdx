...
It composes the rooms — `den`, `hearth`, `prowl`, `sill`, `collar`, `pounce` —
and layers in home-manager, the [nebelung](/reference/palette/) theme, and the
[pounce](/guides/pounce/) app. Each room is also exported individually under
`darwinModules` (`den`, `hearth`, `prowl`, `sill`, `collar`, `pounce`, and
`default` for all of them) if you'd rather cherry-pick.
...
*contributors* use the workshop's `haus ship` to ripple a change all the way
