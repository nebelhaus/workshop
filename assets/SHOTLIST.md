...
| **M4 — fog banner** | barely-moving drift | 1040×260 static | `assets/fog-banner.png` (static frame), **shelved**: the still adds nothing over the current banners. An animated APNG export (900×225, 932 KB) was tried in the org + rice READMEs 2026-07-13 and reverted because its rounded corners rendered opaque black on GitHub's dark background. Retry only with an export whose alpha survives (e.g. animated WebP, per the original brief) and stays ≤ ~1.5 MB / barely moving; then place it in the org-profile + rice child repos from the main workshop checkout. |
